...
# 1. Fetch your secrets directly from Vercel
...
async def vehicle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Use the Secret API Base URL instead of hardcoding it
    ...
    try:
        ...
        try:
            ...
            await status_msg.edit_text("⚠️ Raw Response Failed to parse JSON.")
            return
...
# 3. This is the Webhook endpoint Vercel will trigger when Telegram sends a message
@app.post("/api/index")
async def process_update(request: Request):
    # Initialize the bot in the serverless environment if it hasn't been already
